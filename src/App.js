...
export default function App() {
  ...
  const PARTICIPANT_SLOGANS = {
    "Kenneth Lizano":          "🧊 El más frío en la cima",
    "Mauricio ":               "🔄 Lo tirás y vuelve solo",
    "Mauricio":                "🔄 Lo tirás y vuelve solo",
    "Carlos Mora":             "⚓ Creó el barco y lo capitaneó hasta el podio",
    "Andrés López":            "🎯 Siempre cerca, siempre ahí",
    "Asdru":                   "🃏 El rey del comodín",
    "Fernandinho":             "🎆 Brilló fuerte, se apagó más fuerte",
    "Carlos Lopez":            "💎 Silencioso pero constante",
    "Fcano19":                 "👻 Apareció de la nada y se quedó",
    "Bladimir":                "📱 Siempre con el teléfono perdido pero los puntos encontrados",
    "Bryan Mora":              "🔄 Cambió más jugadores que un técnico de selección",
    "Carlos Sarmiento":        "🎲 Le apuesta a todo menos a lo correcto",
    "Lu":                      "🌙 Tranquila por fuera, calculadora por dentro",
    "Daniel Soto":             "🤷 Sin pronóstico igual suma",
    "Eduardo Salguera":        "⚡ Rápido para adivinar, lento para subir",
    "Esteban CM":              "🏃 Siempre en movimiento en la tabla",
    "Gustavo el manco Ramirez":"🦾 Ni el apodo lo detiene",
    "Josué ":                  "⏰ Siempre justo a tiempo... o tarde",
    "Josué":                   "⏰ Siempre justo a tiempo... o tarde",
    "Leo Fallas ":             "🌪️ Ni rugiendo se mueve de ahí",
    "Leo Fallas":              "🌪️ Ni rugiendo se mueve de ahí",
    "Martin":                  "🎭 El misterioso de la quiniela",
    "Max":                     "✏️ Cambia jugadores más que de opinión",
    "Mike 0606":               "📲 Siempre conectado, siempre pronosticando",
    "Natalia Centeno":         "🌟 La sorpresa silenciosa",
    "Jorge Webb":              "🌍 Visión global, pronósticos locales",
  }
      ...
      {leaderboard.length===0
        ...
        : leaderboard.map((p,i)=>{
          ...
          <div key={p.id} style={{
            ...
            border:isFirst?"1px solid rgba(245,200,66,0.5)":isLast?"1px solid rgba(232,85,85,0.4)":isSecond?"1px solid rgba(192,192,192,0.4)":isThird?"1px solid rgba(205,127,50,0.4)":S.card.border,
            background:isFirst?"linear-gradient(135deg, rgba(245,200,66,0.08), rgba(255,255,255,0.04))":isLast?"linear-gradient(135deg, rgba(232,85,85,0.06), rgba(255,255,255,0.04))":isSecond?"linear-gradient(135deg, rgba(192,192,192,0.08), rgba(255,255,255,0.04))":isThird?"linear-gradient(135deg, rgba(205,127,50,0.08), rgba(255,255,255,0.04))":S.card.background,
            position:"relative"
          }}>
            {isFirst && (
            ...
            <div style={{flex:1}}>
              ...
              {!isFirst && !isSecond && !isThird && !isLast && i!==3 && i!==4 && PARTICIPANT_SLOGANS[p.name] && (
                <div style={{fontSize:10,color:"#6a8caa",marginTop:3,fontStyle:"italic"}}>{PARTICIPANT_SLOGANS[p.name]}</div>
              )}
              {i===3 && PARTICIPANT_SLOGANS[p.name] && (
                <div style={{fontSize:10,color:"#8a8aff",marginTop:3,fontStyle:"italic"}}>{PARTICIPANT_SLOGANS[p.name]}</div>
              )}
              {i===4 && PARTICIPANT_SLOGANS[p.name] && (
                <div style={{fontSize:10,color:"#2e86ab",marginTop:3,fontStyle:"italic"}}>{PARTICIPANT_SLOGANS[p.name]}</div>
              )}
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:22,fontWeight:900,color:isFirst?"#f5c842":isLast?"#e85555":isSecond?"#c0c0c0":isThird?"#cd7f32":"#fff"}}>{p.points}</div>
